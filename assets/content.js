...
CONTENT.miParte = {
 ...
 { slide: "26", t: "Evaluación ATAM", min: 1, ver: "atam",
   say: [
    "Para validar todo esto aplicamos ATAM, el Architecture Tradeoff Analysis Method. No pregunta si la arquitectura es buena o mala, sino si nuestras decisiones sostienen los atributos de calidad del SRS y qué se gana y se pierde con cada una. Por eso no da una nota: da un mapa. Como dice la diapositiva, lo aplicamos sobre la documentación y antes de elegir tecnología.",
    "A la derecha están los nueve pasos. Del 1 al 3 se presentan el método, los controladores del negocio y la arquitectura; del 4 al 6 se identifican los enfoques, se arma el árbol de utilidad y se analiza cada enfoque; el 7 y el 8 amplían los escenarios con una lluvia de ideas y los reanalizan; y el 9 presenta los resultados, que son los que les voy a mostrar."],
 ...
 { slide: "27", t: "Método y controladores del negocio", min: 1, ver: "atam",
   say: [
    "La pregunta guía es la del subtítulo: ¿la coordinación sigue viva cuando el entorno se degrada? A la izquierda, cómo evaluamos: identificamos riesgos y compromisos antes de elegir tecnología, priorizamos por importancia para el negocio y dificultad arquitectónica, y mantenemos la neutralidad tecnológica de esta entrega, evaluando propiedades y no productos.",
    "A la derecha, los siete controladores del negocio, lo que la organización necesita que la arquitectura garantice: coordinar sin reemplazar los sistemas existentes, sobrevivir al pico de demanda, operar con el exterior caído, registrar todo lo que se decide, proteger a las personas afectadas, adaptarse sin rehacer y llegar hasta la reconstrucción.",
    "Abajo, la tensión clave: el evaluador trabaja en campo sin conexión con datos que pueden ser personales. Lo resolvimos con cifrado en el dispositivo, desbloqueo obligatorio, caducidad de la consulta local y borrado remoto."],
 ...
 { slide: "28", t: "Enfoques y árbol de utilidad", min: 1, ver: "atam",
   say: [
    "Aquí pasamos de los atributos a escenarios medibles. Identificamos 16 enfoques arquitectónicos, que son patrones repetidos en varias decisiones; en pantalla hay cinco: puertos y adaptadores, modelo canónico, bandeja de salida transaccional, compartimentación y redundancia multizona. El árbol tiene 7 ramas, una por atributo de calidad del SRS, y de ellas salen 26 escenarios priorizados.",
    "Cada escenario lleva dos letras: importancia, alta si afecta a una función crítica, y dificultad, alta si depende de varias decisiones o de supuestos externos. A la derecha ven una muestra con dos escenarios por rama, desde disponibilidad hasta seguridad y trazabilidad: por ejemplo, la pérdida de una zona, el salto de mil a más de diez mil usuarios, la edición concurrente sin conexión o el intento de alterar la auditoría. Los ocho que son alta y alta se analizaron primero."],
 ...
 { slide: "29", t: "Escenarios clave analizados", min: 1, ver: "atam",
   say: [
    "Cuatro escenarios muestran cómo responde la arquitectura. Si cae el sistema hospitalario, la coordinación continúa: ninguna operación crítica lo llama de forma síncrona, el interruptor de circuito corta las llamadas y se muestra la última información conocida con su antigüedad. Resultado: no riesgo.",
    "Ante un pico súbito, de mil a más de diez mil usuarios, cada compartimento escala por separado y la consulta ciudadana no le quita capacidad a la sala de crisis. No riesgo.",
    "Si un evaluador edita sin conexión algo que un validador ya cambió, el conflicto se detecta y nunca se sobrescribe en silencio; qué versión prevalece es una regla de negocio, por eso dice decisión del cliente. Y si alguien intenta alterar la auditoría, incluso el administrador, la cadena encadenada y sellada lo detecta. No riesgo."],
 ...
 { slide: "30", t: "Resultados de la evaluación", min: 1, ver: "atam",
   say: [
    "Estos son los hallazgos. 23 riesgos: un riesgo dice qué falta decidir o medir, no que algo falle. El ejemplo es la autenticación, la única operación crítica que depende de un sistema externo; aun así, las sesiones abiertas siguen funcionando.",
    "15 no riesgos, decisiones bien justificadas, como el aislamiento de la demanda ciudadana: la consulta pública sale de su propio compartimento y de proyecciones, así que la sala de crisis no la nota.",
    "21 puntos de sensibilidad, parámetros que la tecnología elegida debe fijar, como el número de zonas. Y 14 puntos de compromiso: caché disponible frente a dato desactualizado, que mitigamos mostrando la antigüedad del dato, e inmutabilidad frente al derecho de supresión, que resolvemos destruyendo la clave de cifrado de cada persona. Todo esto se agrupa en 7 temas de riesgo y se atiende con 12 recomendaciones."],
 ...
 { slide: "31", t: "Conclusiones y recomendaciones", min: 1, ver: "atam",
   say: [
    "En conclusión, la coordinación sigue viva cuando el entorno se degrada: si un sistema externo cae, la coordinación sigue; la demanda ciudadana no afecta a la sala de crisis; y cualquier alteración de la auditoría se detecta. 15 de los 26 escenarios dieron no riesgo.",
    "Para la segunda entrega recomendamos acordar con el cliente la política de conflictos de sincronización, levantar los datos del modelo de capacidad, formalizar la contingencia de autenticación y documentar el modelo de operación, con turnos y equipo. Y los puntos de sensibilidad y de compromiso guiarán la selección tecnológica. ATAM no nos dio una nota: nos dio un mapa."],
...
CONTENT.qa = [
 ...
 { tag: "Integración", q: "¿Por qué la sincronización de campo se hace por lotes?",
   a: "Porque en campo no hay conexión. El evaluador acumula su trabajo en el dispositivo y, cuando recupera señal, la app envía todo lo pendiente en un solo lote, sin intervención del usuario (RF-MOV-004). Enviar registro por registro exigiría red permanente y, con una señal débil que se corta, sería lento y fallaría a cada rato. La meta es 50 evaluaciones en menos de 5 minutos con 1 Mbps (AC-REN-005). Es el patrón PIN-06, sincronización por lotes con reconciliación.",
   more: "El receptor deja el lote en una cola persistente y confirma de inmediato, así absorbe la ráfaga de cientos de equipos sin tocar la sala de crisis (compartimento de ingesta de campo, PC-06). Cada registro lleva un identificador generado en el dispositivo (RF-MOV-005): si el lote se reenvía tras un corte, no se duplica (DA-API-03). Los trabajadores comparan la versión base con la vigente y, si alguien la cambió, la marcan como conflicto en vez de sobrescribir (ESC-14). Las fotos viajan aparte, por carga reanudable (DA-ARQ-10)." },
